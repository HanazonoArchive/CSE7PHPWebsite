<?php
define('PROJECT_DB3', $_SERVER['DOCUMENT_ROOT'] . '/CSE7PHPWebsite/public/');
include_once PROJECT_DB3 . "db/DBConnection.php";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action === "create") {
        try {
            ...
            $conn->commit();
            header('Content-Type: application/json');
            echo json_encode(["status" => "success", "message" => "Employee created successfully", "employee_id" => $employee_id, "reload" => true]);
        } catch (Exception $e) {
            $conn->rollBack();
