...
function setupAutocomplete(input) {
  ...
  let savedInputs = JSON.parse(localStorage.getItem(`saved_${fieldName}`)) || [];
  ...
  input.addEventListener("input", () => {
      updateDatalist(dataList, savedInputs.filter(item => item.toLowerCase().includes(input.value.toLowerCase())));
  });
